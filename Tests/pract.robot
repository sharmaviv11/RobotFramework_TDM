...
#        pandas.read_csv(r"C:\Users\sharvive\BHP\Nonabur, Navin - ShareVivek\Centric\FACT_HAULAGE\RAW Data\HAULAGE_27 July_50481.csv")
        ${csv}=         Get file            dest.csv        encoding=UTF-8
        log      ${csv}
        @{data}=    create list     ${csv}
        @{lines}=   split to lines  @{data}
        ${get_line}=    get line    @{data}    -1
        log     ${lines}
        log     ${get_line}
        ${get_variable_value}=      get variable value      ${get_line}



#        FOR     ${data}      IN      ${csv}
#            continue for loop if    ${data} == ORIGINID
#        END
